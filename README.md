...
이 프로젝트는 개인 포트폴리오 용으로 제작된 개인 프로젝트입니다. **'여행 어디가? 웹사이트의 주요 기능(검색, 지역 여행지 소개 등)을** 구현하고, **무한 스크롤, 스켈레톤 UI, 다크 모드을** 추가하여 사용자 경험을 개선했습니다. **Open API(한국관광공사\_국문 관광정보 서비스)를** 활용하여 새로운 여행 정보를 제공하며, **Next.js와 앱 라우팅 방식**을 사용하여 빠른 페이지 전환과 자연스러운 UI 렌더링을 구현했습니다. 동적 데이터 처리를 위해 **React Query와 Zustand**를 활용하여 서버 상태와 클라이언트 상태를 효율적으로 관리했습니다.
